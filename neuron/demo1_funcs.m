...
%     -xx(1,1)+1/(1+exp(-par(2)*(par(7)+par(3)*xx(1,2)+par(4)*xx(2,3))));....
%     par(1)*(-xx(2,1)+1/(1+exp(-par(2)*(par(8)+par(5)*xx(1,3)+par(6)*xx(2,2)))))];
...
% neuron_tau=@()[9,10];
neuron_tau=@()9;
ind_theta_u=7;  % used later for continuation
ind_taus=9; % used later for continuation
